<script>
	...
	import { siteNavLinks, siteNavCta } from '$lib/ui/site-nav';

	fileLogger('src/lib/ui/Navbar.svelte');
				...
				<Button
					...
					className={`group/nav relative overflow-hidden ${
						isActiveLink(link.href)
							? 'shadow-[0_10px_18px_rgba(11,11,11,0.12)]'
							: 'text-[#111111]'
					}`}
				>
					<span class="relative z-10">{link.label}</span>
					<span
						class={`pointer-events-none absolute inset-x-4 bottom-1 h-[2px] rounded-full bg-current transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${
							isActiveLink(link.href)
								? 'scale-x-100 opacity-100'
								: 'scale-x-0 opacity-0 group-hover/nav:scale-x-100 group-hover/nav:opacity-65'
						}`}
					></span>
				</Button>
			{/each}
					...
					{#each siteNavLinks as link}
						<a
							...
							class={`inline-flex w-full items-center justify-start rounded-[1.4rem] px-4 py-3 text-base font-semibold transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#038C25] focus-visible:ring-offset-2 ${
								isActiveLink(link.href)
									? 'bg-[#0B0B0B] text-white shadow-[0_18px_40px_rgba(11,11,11,0.16)]'
